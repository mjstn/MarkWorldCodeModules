...
/*!
 ...
 Modified to use the I2C library by mjstn2011 (arduino forums).
  The I2C library version 5 by Wayne Truchsess was used which
  is ISR safe as it polls and does not use ISR.  
  Look for   I2CMOD for locations modified
  
 ****************************************************/
#include "I2C.h"			// Wayne Truchsess lib that uses poll lib to make this code ISR safe 
#include <Adafruit_MMA8451_PollingI2C.h>

#define I2CMOD				// use undef on this to revert to use of Wire.h ISR mode
...
void Adafruit_MMA8451::writeRegister8(uint8_t reg, uint8_t value) {
  #ifdef I2CMOD
  // Use the I2C polled interface which returns 0 for no error
  if (I2c.write((uint8_t)_i2caddr, (uint8_t)reg, uint8_t(value)) != 0) {
	return;		// Would be nice to indicate error but we maintain compatibility with ported code here
  }
  #else
...
uint8_t Adafruit_MMA8451::readRegister8(uint8_t reg) {
	...
	// Use the I2C polled interface which returns 0 for no error
	if (I2c.read((uint8_t)_i2caddr, (uint8_t)reg, (uint8_t)1, &inBuf[0]) != 0) {
	  return 0;		// Would be nice to indicate error but we maintain compatibility with ported code here
	}
	return inBuf[0];
	#else
    Wire.beginTransmission(_i2caddr);
...
void Adafruit_MMA8451::read(void) {
  ...
  // Use the I2C polled interface which returns 0 for no error
  if (I2c.read((uint8_t)_i2caddr, (uint8_t)MMA8451_REG_OUT_X_MSB, 6, &inBuf[0]) == 0) {
	x = inBuf[0]; x <<= 8; x |= inBuf[1]; x >>= 2;
    y = inBuf[2]; y <<= 8; y |= inBuf[3]; y >>= 2;
    z = inBuf[4]; z <<= 8; z |= inBuf[5]; z >>= 2;
  } else {
	return;		// Would be nice to indicate error but we maintain compatibility with ported code here
  }
  #else 

...
#include <Adafruit_L3GD20_PollingI2C.h>
...
bool Adafruit_L3GD20::begin(l3gd20Range_t rng, byte addr)
{
  ...
  //if ((id != L3GD20_ID) && (id != L3GD20H_ID) && (id != L3GD20AF_ID ))
  //{
  //  return false;
  //}
...
{ 
  ...
  if (_cs == -1) {
    ...
    if (I2c.read(address, (uint8_t)L3GD20_REGISTER_OUT_X_L, 6, &inBuf[0]) == 0) {
	  xlo = inBuf[0];
      xhi = inBuf[1];
      ylo = inBuf[2];
      yhi = inBuf[3];
...
void Adafruit_L3GD20::write8(l3gd20Registers_t reg, byte value)
{
  if (_cs == -1) {
	...
	#ifdef I2CMOD
	// Use the I2C polled interface which returns 0 for no error
    if (I2c.write(address, (uint8_t)reg, uint8_t(value)) != 0) {
	  return;		// Would be nice to indicate error but we maintain compatibility with ported code here
	}
    #else
	Wire.beginTransmission(address);
...
byte Adafruit_L3GD20::read8(l3gd20Registers_t reg)
{
  ...
  if (_cs == -1) {
    // use i2c
	#ifdef I2CMOD
	if (I2c.read(address, (uint8_t)L3GD20_REGISTER_OUT_X_L, (uint8_t)1, &inBuf[0]) == 0) {
	  value = inBuf[0];
	} else {

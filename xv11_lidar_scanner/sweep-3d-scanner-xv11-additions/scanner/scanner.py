...
class Scanner(object):
    ...
    def perform_scan(self):
        ...
        print("Scan: sweeps: ", num_sweeps, " anglePerSweep: ", angle_between_sweeps, " stepsPerMove: ", steps_per_move)

        # Report that the scan is initiating, and start scanning
        self.report_scan_initiated(num_sweeps)
        self.device.start_scanning()

        # put a multi second timeout on the get_scans() method in case it hangs
        ...
        for scan_count, scan in enumerate(self.device.get_scans()):
            ...
            if not rotated_already:
                ...
                # MODIFICATION for more torque!  mjstn2011
                self.base.move_steps(steps_per_move)
                rotated_already = True
                continue

            # Here is the 'magic' math that generates one slice of the pointcloud.
            # Export the single 2D scan just taken as one X,Y,Z,sigStrength row of output
            print(valid_scan_index, angle_between_sweeps,self.settings.get_mount_angle())
    ...
    def idle(self):
        ...
        # We are not going to implement stopping the sweep
    ...
    def check_get_scan_timeout(self):
        ...
        if not self.received_scan:
            ...
            raise ValueError("getScans() never returned... aborting")
...
def main(arg_dict):
    ...
    if (use_xv11 == True):
        print("Create sweep device with X11 Lidar")
    with sweep_helpers.create_sweep_w_error('/dev/ttyACM0', use_dummy, use_xv11) as (sweep, err):
        if err:
            output_json_message(
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dz', '--dead_zone',
                        ...
                        default=135,
                        required=False)
    default_filename = "ScanOut_" + datetime.datetime.fromtimestamp(
        time.time()).strftime('%Y-%m-%d_%H-%M-%S') + '.asc'
    ...
    parser.add_argument('-d', '--use_dummy',
                        ...
                        action='store_true',
                        required=False)
    parser.add_argument('-x', '--use_xv11',
                        help='Use the x11 lidar hardware',
                        default=False,
                        action='store_true',
                        required=False)
